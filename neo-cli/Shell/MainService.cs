...
using EpicChain.Core;
using EpicChain.Implementations.Blockchains.LevelDB;
using EpicChain.Implementations.Wallets.EntityFramework;
using EpicChain.Implementations.Wallets.NEP6;
using EpicChain.IO;
using EpicChain.IO.Json;
using EpicChain.Network;
using EpicChain.Network.Payloads;
using EpicChain.Network.RPC;
using EpicChain.Services;
...
namespace EpicChain.Shell
{
    internal class MainService : ConsoleServiceBase
    {
        ...
        private bool OnShowEpicPulseCommand(string[] args)
        {
            if (Program.Wallet == null)
            {
                Console.WriteLine($"Please open a wallet");
        ...
        private bool OnShowCommand(string[] args)
        {
            ...
            {
                case "epicpulse":
                    return OnShowEpicPulseCommand(args);
                case "node":
        ...
        private void LevelDBBlockchain_ApplicationExecuted(object sender, ApplicationExecutedEventArgs e)
        {
            ...
            json["epicpulse_consumed"] = e.EpicPulseConsumed.ToString();
