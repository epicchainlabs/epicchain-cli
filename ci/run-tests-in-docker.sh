#!/bin/bash
#
# This script is run inside the Docker container and tests epicchain-cli
#
set -e

cd /opt/epicchain-cli

# Run tests with expect
expect /opt/ci/test-epicchain-cli.expect

# Start epicchain-cli in background for additional JSON-RPC tests
screen -dmS node1 bash -c "dotnet epicchain-cli.dll --rpc"
